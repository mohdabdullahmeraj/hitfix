...
from layers import SAGEConv, GATConv, AGDNConv, MemAGDNConv,MemoryAttentionGraphConvLayer, DGLGraphModel
...
class MemoryAttentionGraph(Module):
    def __init__(self, in_feats, n_hidden, out_feats, n_layers,
        ...
        self.convs = ModuleList()
        self.norms = ModuleList()
        self.convs.append(MemoryAttentionGraphConvLayer(in_feats, n_hidden, num_heads, K, attn_drop=attn_drop, edge_feats=n_edge_hidden, weight_style=weight_style, residual=True, bias=False))
        self.norms.append(BatchNorm1d(num_heads * n_hidden))
        if in_edge_feats > 0:
        ...
        for _ in range(n_layers - 2):
            self.convs.append(
                MemoryAttentionGraphConvLayer(n_hidden * num_heads, n_hidden, num_heads, K, attn_drop=attn_drop, edge_feats=n_edge_hidden, weight_style=weight_style, residual=True, bias=False))
            self.norms.append(BatchNorm1d(num_heads * n_hidden))
            if in_edge_feats > 0:
                self.edge_encoders.append(Linear(in_edge_feats, n_edge_hidden))
        self.convs.append(MemoryAttentionGraphConvLayer(n_hidden * num_heads, out_feats, num_heads, K, attn_drop=attn_drop, edge_feats=n_edge_hidden, weight_style=weight_style, residual=True, bias=False))
        if in_edge_feats > 0:
            self.edge_encoders.append(Linear(in_edge_feats, n_edge_hidden))
    ...
    def forward(self, graph, feat, edge_feat=None):
        ...
        for i, conv in enumerate(self.convs[:-1]):
            ...
            h = conv(graph, h, edge_features=edge_feat).flatten(1)
            # if self.residual:
            #     if h_last.shape[-1] == h.shape[-1]:
            #         h += h_last
            h = self.norms[i](h)
        ...
        h = self.convs[-1](graph, h, edge_features=edge_feat).mean(1)
        # if self.residual:
        #     if h_last.shape[1] == h.shape[1]:
        #         h = h + h_last
        h += self.bias
...
class ProposedModel(torch.nn.Module):
    ...
    def forward(self, g, x, edge_feat=None):
